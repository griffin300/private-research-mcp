...
For ordinary questions, configured query counts are ceilings rather than targets. Quick performs at most one targeted expansion, Standard stages up to four across two rounds, and Deep stages up to eight across four rounds, with at most one simplified recovery search per request and never more than the configured total query budget. Expansion stops when query-aligned primary evidence is strong or after repeated low-value rounds. Recognized public identifiers (for example RFC/PEP numbers, a Python release, or a named project manual) can add up to three locally derived canonical URLs; those pages still use the isolated `tor-fetch` path and replace noisy fetch candidates inside the existing page budget. Evidence uses absolute cross-source relevance, a query-relative authority preference, exact source slices, and soft source diversity instead of filling the context with one record from every page. `auto` selects at least Standard and escalates analysis, current information, source requests, conflicts, and architecture questions to Deep; a simple definition or two-item distinction remains Standard. Query, result, page, passage, round, browser, and wall-clock budgets are bounded settings exposed as `PRM_QUICK_*`, `PRM_STANDARD_*`, and `PRM_DEEP_*` variables.
...
SQLite and caches live in the `research-data` Docker volume. Retention is enforced at startup and every five minutes while the service runs; `PRM_CACHE_RETENTION_DAYS=0` disables query-derived database writes entirely. URL-bearing HTTP client logs are suppressed, and SearXNG's Docker logging driver is disabled because engine errors can contain the query. The remaining normal log stream contains opaque request IDs, not raw queries. Call `clear_local_data` with `confirm: true` or remove project data and the volume explicitly:

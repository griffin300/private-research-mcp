...
The `research-data` volume contains SQLite caches/evidence. Default retention is seven days. Cleanup runs at startup and every five minutes while the MCP service is active; it removes aged request fingerprints as well as expired cache, evidence metadata, and FTS rows. SQLite `secure_delete` is enabled on every database connection, and each cleanup requests a truncating WAL checkpoint; a checkpoint delayed by an active reader is retried on the next sweep. A zero-day purge also runs `VACUUM` and a final truncating checkpoint to remove freed database pages. Setting `PRM_CACHE_RETENTION_DAYS=0` disables all query-derived request, evidence, FTS, and cache writes. Search history and raw query logs are off. URL-bearing HTTP client logs are suppressed, and SearXNG's Docker logging driver is disabled because engine failures can otherwise render the query URL. Normal request records contain an ephemeral-keyed, truncated HMAC-SHA-256 fingerprint and opaque request ID, preventing offline dictionary testing after the process key is gone. URLs with credential-like query parameters bypass page/extraction caches and are redacted from returned diagnostics. Browser profiles/cookies are never persisted.

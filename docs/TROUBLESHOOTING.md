...
If the tool reports completion but the model produces no answer, first reload the MCP integration and call `search_status`. `interactive_limits` should report a 105-second deadline, a five-second repeat-search cooldown, and 9,000/10,000/12,000-character Quick/Standard/Deep budgets. The compact result tells the model to answer immediately and not call another search tool in the same turn. The server also suppresses an immediate duplicate or parallel search with a tiny control response; together these controls avoid accumulating multiple large tool results until no generation context remains.
